...
def card(name):
    ...
    style=' style="font-feature-settings:&quot;ss01&quot; 1"' if name=='O.plain' else ''
    return f'<article><h2>{html.escape(char)} <small>{name}</small></h2><div class="samples"><section><label>生成見本</label>{refsvg}</section><section><label>現行 輪郭拡大</label>{actual(name,"Current")}</section></div><div class="sizes"{style}><span style="font-size:24px">{html.escape(char)}</span><span style="font-size:48px">{html.escape(char)}</span><span style="font-size:72px">{html.escape(char)}</span></div><footer>advance {d["advance"]} · {html.escape(d["review"])}</footer></article>'
